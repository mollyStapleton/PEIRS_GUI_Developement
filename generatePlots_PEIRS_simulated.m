function [out1, out2, out3, out4, out_low, out_high] = generatePlotData_PEIRS_simulated(s_safe, s_risky, alpha_q, alpha_s, beta, omega, distType)
...
for i = 1:iters
    ...
    % S values refer to the agents belief of the underlying spread of reward values
    % start at >0 because of individual starting beliefs
    S1 = s_safe; S2 = s_risky; S3 = s_safe; S4 = s_risky;
...
elseif distType == 2 
        ...
        % update of the belief of underlying spread of distribution
        S1(t) = S1(t-1)+alpha_s*(abs(R1(t)-Q1(t-1)) - S1(t-1));
        S2(t) = S2(t-1)+alpha_s*(abs(R2(t)-Q2(t-1)) - S2(t-1));
        S3(t) = S3(t-1)+alpha_s*(abs(R3(t)-Q3(t-1)) - S3(t-1));
        S4(t) = S4(t-1)+alpha_s*(abs(R4(t)-Q4(t-1)) - S4(t-1));
